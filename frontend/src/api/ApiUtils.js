...
function isAbortedOrSafeRequestError(error) {
    return axiosRetry.isSafeRequestError(error) || error.code === 'ECONNABORTED'
}
...
export const setToken = (isLoggedIn) => {
    ...
    if (localStorage.getItem("jwtToken") !== 'undefined') {
        api.defaults.headers.common['Authorization'] = 'Bearer '.concat(JSON.parse(localStorage.getItem("jwtToken")))
    }
}

export async function httpRequest(method, url, body, contentType, params, config) {
